...
in {
  ...
  in [
    ...
    (mkCommand
      currentSystem
      "showChaoticConfig"
      "outputs a JSON with all chaos module configurations"
      (with inputs.nixpkgs; [bat jq])
      ''
        nix eval --json "$PRJ_ROOT#nixosConfigurations.${collectedTargetName}.config.chaos" | jq | bat --show-all
      ''
      {})
  ];
}
